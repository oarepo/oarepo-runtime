...
def reindex(recreate, queue, *args, **kwargs):
    if recreate:
        index = Records2Record.index._name
    ...
    id_generator = (
        x[0]
        for x in db.session.query(Records2Record.model_cls.id).filter(
            Records2Record.model_cls.is_deleted.is_(False)
        )
    )
    ...
    if queue:
        ...
        for rec_id in tqdm.tqdm(ids, "Indexing records"):
            record = Records2Record.get_record(rec_id)
