...
def validate_identifier(value):
    try:
        ...
        if value["scheme"].lower() == "isbn":
            ...
            if (
                original_identifier and not canonical_isbn
            ):  # just check in case it returns empty string
                raise ValueError()
        ...
        raise ValidationError(
            {
                "identifier": _(
                    "Invalid value %(identifier)s of identifier type %(type)s"
                )
                % {"identifier": value["identifier"], "type": value["scheme"]}
            }
        )
    return value
